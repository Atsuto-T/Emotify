...
# Note: process photos directly when time permits.
            if results.multi_face_landmarks:
                ...
                for fl in results.multi_face_landmarks:
                    ...
                    """Preprocesses for keras model"""
                    # reshape cur_face to shape (48, 48, 3)
                    cur_face = cv2.resize(cur_face, (48, 48))
                    # BGR to RGB
                    cur_face = cv2.cvtColor(cur_face, cv2.COLOR_BGR2RGB)
                    # Normalize the pixel values
                    cur_face = cur_face.astype('float') / 255.0
                    # change cur_face to numpy array
                    cur_face = img_to_array(cur_face)
                    # DIY_model's input 0 of layer expect shape=(None, 48, 48, 3)
                    # expand dimention for DIY_model: shape=(None, 48, 48, 3)
                    cur_face = np.expand_dims(cur_face, axis=0)

                    '''Extracts features from the preprocessed face image using keras model(.h5)'''
                    # DIY model's layers[-2] is Relu and output shape=(1, 512)
                    backbone_layer = DIY_model.layers[-2]

                    # input shape=(None, 48, 48, 3), output shape=(1, 512)
                    features = Model(inputs=DIY_model.input, outputs=backbone_layer.output)(cur_face)
